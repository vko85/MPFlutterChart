...
class XAxisRendererHorizontalBarChart extends XAxisRenderer {
  ...
  void computeSize() {
    axisLabelPaint = PainterUtils.create(
        axisLabelPaint,
        null,
        axisLabelPaint!.text!.style!.color ?? ColorUtils.holoGreenDark,
        xAxis!.textSize,
  ...
  void renderAxisLabels(Canvas c) {
    ...
    } else if (xAxis!.position == XAxisPosition.topInside) {
      pointF.x = 1.0;
      pointF.y = 0.5;
      drawLabels(c, viewPortHandler!.contentRight(), pointF, xAxis!.position);
    ...
    } else if (xAxis!.position == XAxisPosition.bottomInside) {
      pointF.x = 1.0;
      pointF.y = 0.5;
      drawLabels(c, viewPortHandler!.contentLeft(), pointF, xAxis!.position);
    } else {
  ...
  void drawLabels(
    ...
    List<double?> positions =
        List.filled(xAxis!.entryCount * 2, null, growable: false);
    ...
    for (int i = 0; i < positions.length; i += 2) {
      ...
      if (viewPortHandler!.isInBoundsY(y)) {
        String label = xAxis!
            .getValueFormatter()!
            .getAxisLabel(xAxis?.entries[i ~/ 2] ?? 0, xAxis);
  ...
  void drawGridLine(Canvas c, double x, double? y, Path path) {
    path.moveTo(viewPortHandler!.contentRight(), y!);
    path.lineTo(viewPortHandler!.contentLeft(), y);
  ...
  void renderAxisLine(Canvas c) {
    ...
    if (xAxis!.position == XAxisPosition.top ||
        xAxis!.position == XAxisPosition.topInside ||
        xAxis!.position == XAxisPosition.bothSided) {
      c.drawLine(
          Offset(
    ...
    if (xAxis!.position == XAxisPosition.bottom ||
        xAxis!.position == XAxisPosition.bottomInside ||
        xAxis!.position == XAxisPosition.bothSided) {
      c.drawLine(
  ...
  void renderLimitLines(Canvas c) {
    ...
    for (int i = 0; i < limitLines.length; i++) {
      ...
      if (label != null && label.isNotEmpty) {
        ...
        if (position == LimitLabelPosition.rightTop) {
          axisLabelPaint = PainterUtils.create(
        ...
        } else if (position == LimitLabelPosition.rightBottom) {
          axisLabelPaint = PainterUtils.create(
              axisLabelPaint, label, l.textColor, l.textSize);
          axisLabelPaint?.layout();
          var offset = Offset(
          ...
          CanvasUtils.renderLimitLabelBackground(c, axisLabelPaint, offset, l);
          axisLabelPaint?.paint(c, offset);
        } else if (position == LimitLabelPosition.rightCenter) {
          axisLabelPaint = PainterUtils.create(
              axisLabelPaint, label, l.textColor, l.textSize);
          axisLabelPaint?.layout();
          var offset = Offset(
          ...
          CanvasUtils.renderLimitLabelBackground(c, axisLabelPaint, offset, l);
          axisLabelPaint?.paint(c, offset);
        } else if (position == LimitLabelPosition.leftCenter) {
          axisLabelPaint = PainterUtils.create(
              axisLabelPaint, label, l.textColor, l.textSize);
          axisLabelPaint?.layout();
          ...
          CanvasUtils.renderLimitLabelBackground(c, axisLabelPaint, offset, l);
          axisLabelPaint?.paint(c, offset);
        } else if (position == LimitLabelPosition.leftTop) {
          axisLabelPaint = PainterUtils.create(
              axisLabelPaint, label, l.textColor, l.textSize);
          axisLabelPaint?.layout();
          var offset = Offset(
