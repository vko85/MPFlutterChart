...
//   StaticLayout _centerTextLayout;
  ...
  // ignore: unused_field
  final Rect _centerTextLastBounds = Rect.zero;
  final List<Rect> _rectBuffer = List.empty(growable: true)
    ..add(Rect.zero)
    ..add(Rect.zero)
    ..add(Rect.zero);
    ...
    for (int i = 0; i < dataSets.length; i++) {
      ...
      for (int j = 0; j < entryCount; j++) {
        ...
        final bool drawXOutside =
            drawEntryLabels && xValuePosition == ValuePosition.outsideSlice;
        final bool drawYOutside =
            drawValues && yValuePosition == ValuePosition.outsideSlice;
        final bool drawXInside =
            drawEntryLabels && xValuePosition == ValuePosition.insideSlice;
        final bool drawYInside =
            drawValues && yValuePosition == ValuePosition.insideSlice;
        ...
        if (drawXOutside || drawYOutside) {
          ...
          if (dataSet.getValueLineColor() != ColorUtils.colorNone) {
        ...
        if (entry.mIcon != null && dataSet.isDrawIconsEnabled()) {
          ...
          CanvasUtils.drawImage(
              c, Offset(x, y), entry.mIcon!, const Size(15, 15), drawPaint!);
        }
...
//    final Rect highlightedCircleBox = _drawHighlightedRectF;
    ...
    for (int i = 0; i < indices!.length; i++) {
      ...
      IPieDataSet? set = _painter!
          .getData()!
          .getDataSetByIndex(indices[i].dataSetIndex??0) as IPieDataSet?;
