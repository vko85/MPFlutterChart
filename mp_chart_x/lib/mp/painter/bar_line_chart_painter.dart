...
abstract class BarLineChartBasePainter<
  ...
  void compute() {
    ...
    if (_axisLeft!.enabled) {
      _axisRendererLeft?.computeAxis(_axisLeft!.axisMinimum ?? 0,
          _axisLeft!.axisMaximum ?? 0, _axisLeft!.inverted);
    }

    if (_axisRight!.enabled) {
      _axisRendererRight?.computeAxis(_axisRight!.axisMinimum ?? 0,
          _axisRight!.axisMaximum ?? 0, _axisRight!.inverted);
    }
