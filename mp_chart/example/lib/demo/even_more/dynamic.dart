...
class EvenMoreDynamicState extends ActionState<EvenMoreDynamic>
  ...
  void _initController() {
    ...
    controller = LineChartController(
        ...
        infoBgColor: ColorUtils.holoGreenLight,
        description: desc);
  }
  ...
  final List<Color> colors = ColorUtils.vordiplomColors;
  var random = Random(1);
...
//        randomDataSetIndex);
    ...
    data.addEntry(
        Entry(x: randomSet?.getEntryCount().toDouble() ?? 0, y: value),
        randomDataSetIndex);
  ...
  void _addDataSet() {
    ...
    if (data == null) {
      ...
      int amount = data.getDataSetByIndex(0)?.getEntryCount() ?? 0;
  ...
  void _removeDataSet() {
    LineData? data = controller.data;
    if (data != null) {
      var item = data.getDataSetByIndex(data.getDataSetCount() - 1);
      if (item != null) {
        data.removeDataSet1(item);
      }
    }
  }
}
