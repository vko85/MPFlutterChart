...
import 'package:mp_chart_x/mp/core/render/axis_renderer.dart';
...
class YAxisRenderer extends AxisRenderer {
  ...
  void renderAxisLabels(Canvas c) {
    ...
    if (dependency == AxisDependency.left) {
      if (labelPosition == YAxisLabelPosition.outsideChart) {
  ...
  void drawYLabels(
    ...
    for (int i = from; i < to; i++) {
      ...
      if (axisDependency == AxisDependency.left) {
        if (position == YAxisLabelPosition.outsideChart) {
  ...
  List<double?> mGetTransformedPositionsBuffer =
      List.filled(2, null, growable: false);
  ...
  List<double?> getTransformedPositions() {
    if (mGetTransformedPositionsBuffer.length != _yAxis!.entryCount * 2) {
      mGetTransformedPositionsBuffer =
          List.filled(_yAxis!.entryCount * 2, null, growable: false);
    }
  ...
  final Path _drawZeroLinePath = Path();
